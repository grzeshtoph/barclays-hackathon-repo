/**
 * Barclays 2022 CBDC Hackathon API
 * API for the Barclays 2022 CBDC Hackathon.  Please ensure you include your API key in the `x-api-key` header for your requests.
 *
 * OpenAPI spec version: 0.0.9
 *
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

export interface OwnedAccount {
  accountId?: number;
  accountStatus?: OwnedAccount.AccountStatusEnum;
}
export namespace OwnedAccount {
  export type AccountStatusEnum = 'OPEN' | 'CLOSED';
  export const AccountStatusEnum = {
    OPEN: 'OPEN' as AccountStatusEnum,
    CLOSED: 'CLOSED' as AccountStatusEnum,
  };
}
